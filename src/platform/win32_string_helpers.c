...
{
    oc_str16 res = { 0 };
    res.len = 1 + MultiByteToWideChar(CP_UTF8, 0, s.ptr, (int)s.len, NULL, 0);
    res.ptr = oc_arena_push_array(arena, u16, res.len);
    MultiByteToWideChar(CP_UTF8, 0, s.ptr, (int)s.len, res.ptr, (int)res.len);
    res.ptr[res.len - 1] = '\0';
    return (res);
}
...
{
    oc_str8 res = { 0 };
    res.len = WideCharToMultiByte(CP_UTF8, 0, s.ptr, (int)s.len, NULL, 0, NULL, NULL);
    res.ptr = oc_arena_push_array(arena, u8, res.len + 1);
    WideCharToMultiByte(CP_UTF8, 0, s.ptr, (int)s.len, res.ptr, (int)res.len, NULL, NULL);
    res.ptr[res.len] = '\0';
    return (res);
}
