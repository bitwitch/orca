...
def ensure_angle(version):
    ...
    if not verify_angle(checkfiles):
        # NOTE(shaw): the GITHUB_ACTIONS environment variable is defined by default in any workflow
        if "GITHUB_ACTIONS" in os.environ:
            print("error: failed to find angle libs during a CI build, there is probably a bug in the build-angle workflow")
            exit(1)
        else:
